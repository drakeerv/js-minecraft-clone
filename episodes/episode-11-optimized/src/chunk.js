...
class Chunk {
    ...
    async updateSubchunkMeshes() {
        await Promise.all(Object.values(this.subchunks).map(subchunk => subchunk.updateMesh()));
    }
    ...
    async updateMesh() {
        ...
        await Promise.all(Object.values(this.subchunks).map(async subchunk => {
